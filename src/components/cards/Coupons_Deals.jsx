    ...
    <>
      ...
      >
        ...
        <div className="relative w-[120px] h-[100px] flex-shrink-0 overflow-hidden rounded-lg bg-white">
          ...
          />
          {disabled && <div className="absolute inset-0 bg-black bg-opacity-50 rounded-lg" />}
        </div>
        ...
        <div className="flex flex-col justify-between h-full flex-1 overflow-hidden">
          ...
          {/* Action Button */}
          <button
            onClick={dealCategory === "coupon" ? handleCardClick : handleDealClick}
            disabled={disabled}
            className={`
             cursor-pointer mt-3 self-start px-4 py-1.5 text-sm rounded-md shadow
              ${disabled ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-[#E9E4F4] text-[#592EA9] hover:bg-[#dcd4ee]"}
            `}
          >
            {dealCategory === "coupon" ? "Coupon Code" : "View"}
          </button>
        </div>
      </div>
